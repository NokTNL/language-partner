...
import {
  SPostSpeechMessageRequestBody,
  type PostSpeechMessageRequestBody,
  type PostSpeechMessageResponseBody,
} from "../types/api.ts";
...
router.post<{}, PostSpeechMessageResponseBody | string, unknown>(
  "/chat",
  async (req, res) => {
    if (IS_MOCK_MODE) {
      await delay(500);
    ...
    const { user_input, previous_response_id, lessons } =
      SPostSpeechMessageRequestBody.parse(req.body);
